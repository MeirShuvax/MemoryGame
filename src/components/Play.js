...
//         const playerName = gameResult.Name.toLowerCase();

//         // Find existing player entry if it exists
//         const playerIndex = leaderboard.findIndex(entry => entry.Name.toLowerCase() === playerName);

//         if (playerIndex !== -1) {
//             // Update score only if the new score is higher
//             if (gameResult.Score > leaderboard[playerIndex].Score) {
//                 leaderboard[playerIndex].Score = gameResult.Score;
//                 leaderboard[playerIndex].Steps = gameResult.Steps;
//             }
//         } else {
...
// Simple encoding function (e.g., base64 encoding)
const encodeIndex = (index) => btoa(index);
const decodeIndex = (encodedIndex) => parseInt(atob(encodedIndex));
...
function Play() {
    ...
    useEffect(() => {
        ...
        const imageIndices = [];
        for (let i = 0; i < numPairs; i++) {
            imageIndices.push(encodeIndex(i));
            imageIndices.push(encodeIndex(i));
        }
    ...
    const handleCardClick = (index) => {
        ...
        if (newSelectedCards.length === 2) {
            ...
            if (decodeIndex(shuffledImages[firstIndex]) === decodeIndex(shuffledImages[secondIndex])) {
                // Cards match, keep them revealed
                setSelectedCards([]);
                // Check if all cards are revealed
                if (newRevealed.every(card => card === true)) {
        ...
        <div className="play-container">
            ...
            <Container>
                ...
                <Row className="justify-content-center">
                    ...
                    >
                        {shuffledImages.map((encodedImageIndex, index) => {
                            const isRevealed = revealed[index];
                            const imageIndex = decodeIndex(encodedImageIndex);
                            return (
                                <Card
                                    key={index}
                                    className="bg-light text-dark"
